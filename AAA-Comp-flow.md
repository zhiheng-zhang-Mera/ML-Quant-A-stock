...
> **系统概述**：该量化交易系统利用先进的机器学习和稳健的贝叶斯统计学，在多资产投资组合中进行安全的资金分配。以下是这五个文件如何协同处理数据并执行交易的完整生命周期流程：
...
$$\mathrm{Deflator}_{\text{平减指数}} = (1.0 + R_{\mathrm{annual\_inflation},\,\text{年通胀率}})^{\frac{T_{\mathrm{max},\,\text{最大日期}} - T_{\mathrm{current},\,\text{当前日期}}}{365.25}}$$

$$\mathrm{Price}_{\mathrm{real},\,\text{实际价格}} = \mathrm{Price}_{\mathrm{nominal},\,\text{名义价格}} \times \mathrm{Deflator}_{\text{平减指数}}$$
...
$$I_{\mathrm{train\_end},\,\text{训练截止索引}} = N_{\mathrm{total\_dates},\,\text{总日期数}} - 1 - P_{\mathrm{embargo},\,\text{禁运期}}$$
...
$$E_i = \max\left(\hat{q}_{\mathrm{low},\,\text{下分位数}}(X_i) - y_i, \, y_i - \hat{q}_{\mathrm{high},\,\text{上分位数}}(X_i)\right)$$

$$Q_{\mathrm{error\_threshold},\,\text{误差校准阈值}} = \mathrm{Percentile}_{95}\left(\{E_i\}\right)$$
...
$$\mathrm{Bounds}_{\mathrm{floor},\,\text{符合性下限}} = \hat{\mathrm{y}}_{\mathrm{low}} - Q_{\mathrm{error\_threshold}}$$

$$\mathrm{Bounds}_{\mathrm{ceiling},\,\text{符合性上限}} = \hat{\mathrm{y}}_{\mathrm{high}} + Q_{\mathrm{error\_threshold}}$$

$$W_{\mathrm{heteroskedastic},\,\text{动态异方差幅宽}} = \mathrm{Bounds}_{\mathrm{ceiling}} - \mathrm{Bounds}_{\mathrm{floor}}$$
...
$$\max_{w} \text{Sharpe Ratio} = \frac{w^T \cdot R_{\mathrm{BL},\,\text{贝叶斯后验收益}} - R_{f,\,\text{无风险利率}}}{\sqrt{w^T \cdot \Sigma_{\mathrm{robust},\,\text{稳健协方差}} \cdot w} + 10^{-8}}$$
...
> **环境准备说明**：在执行流水线管道前，确保您的运行环境包含以下强制依赖项。可在终端中通过以下命令安装：
...
> **目录变更说明**：初始化启动后，系统将自动在您的当前工作目录下建立以下生产输出目录结构：
...
> **解读前言**：当流水线执行完毕后，它会在控制台输出中英双语核心决策看板，并落盘 `multi_asset_llm_payload.json` 文件。以下是生产主管或上游大模型（LLM Agent）应该如何解读和应用这些核心数据指标的指南。
